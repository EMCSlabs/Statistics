...
anova(fit5)
---------------------------------------------------------------
# generalized linear model w/o random effects (glm fnc) 
# family objects: binomial,gaussian,Gamma,inverse.gaussian,poisson,quasi,quasibinomial,quasipoisson

fit4glm = glm(dur~Tone*Condition,all,family = gaussian)
fit4glm  # coefficients
anova(fit4glm) # Analysis of Deviance
anova(aov(fit4glm)) # Analysis of Variance
...
fit6glm = glmer(dur~Tone+(1|Subject), all, family = gaussian) 
fit6glm # std dev of random effects & coefficeints of fixed effects
anova(fit6glm) # Analysis of Variance
...
lsmeans(fit6glm, pairwise~Tone, adjust = "tukey") # lsmeans with glmer fit
